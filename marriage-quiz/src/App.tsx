...
const questions: Question[] = [
  ...
  },
  {
    ...
    type: "single",
    prompt:
      "생활비를 어떻게 나눌지 가장 현실적인 방식은 무엇이라고 생각하나요?",
    description:
...
function evaluateAnswer(question: Question, answer: string): AnswerResult {
  ...
  if (question.type === "single") {
    return answer === question.correctAnswer
      ...
      : {
          status: "incorrect",
          message:
            "다시 이야기해 보세요. 서로의 상황에 맞는 선택인지 점검하세요.",
        };
  }
  ...
  return {
    status: "incorrect",
    message:
      "조율 기준과 분담 방식이 보이지 않아요. 조금 더 구체적으로 작성해 주세요.",
  };
}
...
function App() {
  ...
  const completedCount = useMemo(
    () =>
      questions.filter((question) => results[question.id]?.status === "correct")
        .length,
    [results]
  );
  ...
  const handleCheckAnswers = () => {
    const nextResults = questions.reduce<Record<string, AnswerResult>>(
      (acc, question) => {
      ...
      <Page>
        <PageHeader>
          <div>
            ...
            <Subtitle>
              질문 데이터는 추후 연결됩니다. 지금은 객관식 · 주관식 정답 체크
              흐름과 공통 UI를 살펴보세요.
            </Subtitle>
          </div>
          <Progress
